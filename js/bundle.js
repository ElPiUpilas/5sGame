...
function computeDirtBlobs(seed, rect) {
  ...
  const blobCount = 18 + Math.floor(rng() * 8);
  ...
  // ── Hatch lines ──────────────────────────────────────
  const lineCount = 12 + Math.floor(makeRng(seed, 'line-count')() * 8);
  for (let i = 0; i < lineCount; i++) {
    ...
    const x1 = rx * rect.w;
    const y1 = ry * rect.h;
    const len = 30 + rlen * 90;
    const angle = rang * Math.PI;
    const x2 = x1 + Math.cos(angle) * len;
    const y2 = y1 + Math.sin(angle) * len;
    const w = 1.5 + makeRng(seed, 'line-w', i)() * 3;
    blobs.push({ type: 'line', x1, y1, x2, y2, w, opacity: 0.20 + makeRng(seed, 'line-op', i)() * 0.30 });
  }

  // ── Splatter drops ───────────────────────────────────
  const dropCount = 20 + Math.floor(makeRng(seed, 'drop-count')() * 15);
  for (let i = 0; i < dropCount; i++) {
    const x = makeRng(seed, 'drop-x', i)() * rect.w;
    const y = makeRng(seed, 'drop-y', i)() * rect.h;
    const r = 2 + makeRng(seed, 'drop-r', i)() * 6;
    const op = 0.35 + makeRng(seed, 'drop-op', i)() * 0.45;
    blobs.push({ type: 'drop', x, y, r, opacity: op });
  }

  // ── Drip streaks ─────────────────────────────────────
  const dripCount = 6 + Math.floor(makeRng(seed, 'drip-count')() * 5);
...
function buildDirtLayer(seed, rect) {
  ...
  for (const b of computeDirtBlobs(seed, rect)) {
    if (b.type === 'blob') {
      ...
      line.setAttribute('x1', b.x1.toFixed(1)); line.setAttribute('y1', b.y1.toFixed(1));
      line.setAttribute('x2', b.x2.toFixed(1)); line.setAttribute('y2', b.y2.toFixed(1));
      line.setAttribute('stroke', '#1a1c2e');
      line.setAttribute('stroke-width', b.w.toFixed(1));
      line.setAttribute('stroke-opacity', b.opacity.toFixed(3));
...
function startRound(onFinish) {
  ...
  // Dirty state — apply immediately if dirty, otherwise no dirt
  // For Shine (cleanAnim), apply dirty first so we can animate it away.
  if (round.dirty || round.cleanAnim) {
    boardEl.classList.add('is-dirty');
  }

  // Measure after CSS class applied (next frame not required; getBoundingClientRect is immediate)
  const rect = boardEl.getBoundingClientRect();
  const boardSize = { w: rect.width, h: rect.height };
  ...
  // Decide: arrancar timer inmediatamente, o (para Shine) esperar animación
  if (round.cleanAnim) {
    // Pequeño delay para que el usuario vea el estado sucio,
    // luego se limpia y arranca el timer.
...
function buildFullBoard(round, rect) {
  ...
  for (const item of layout) {
    ...
    btn.style.zIndex = String(60 - item.value);
